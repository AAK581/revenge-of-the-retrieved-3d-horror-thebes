...
export const BACKEND_CANISTER_ID = 44072137928639;
...
/// Encode N `text` arguments: `(text, text, ...)`.
///
/// `set_username` takes two, and the starter client only had the one-arg
/// form. The type table stays empty because `text` is a primitive — the
/// argument COUNT and one type byte per argument is the whole difference.
export function encodeTexts(...ss: string[]): string {
  const bytes: number[] = [...MAGIC, 0, ss.length];
  for (let i = 0; i < ss.length; i++) bytes.push(TYPE_TEXT_SLEB);
  for (const s of ss) {
    const utf8 = new TextEncoder().encode(s);
